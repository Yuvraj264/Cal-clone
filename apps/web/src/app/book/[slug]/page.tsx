...
import { Clock, Globe, CalendarRange, ArrowLeft } from 'lucide-react';
...
export default function PublicBookingPage({ params }: PublicBookingProps) {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      try {
        setLoading(true);
        // Call backend event type resolver
        const res = await bookingClient.get(`/event-types`);
        const matchingEvent = res.data.data.find(
          (et: any) => et.slug.toLowerCase() === slug.toLowerCase()
        );
  ...
  const handleBookingSubmit = async (formData: { bookerName: string; bookerEmail: string }) => {
    try {
      const res = await bookingClient.post('/bookings', {
        eventTypeSlug: slug,
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 dark:bg-black py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
        ...
        <div className="md:w-1/3 flex flex-col gap-4 border-b md:border-b-0 md:border-r border-gray-100 dark:border-gray-800 pb-6 md:pb-0 md:pr-8">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-gray-500">
              Host: User Account (host@calclone.com)
            </p>
          </div>
